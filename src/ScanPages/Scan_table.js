...
const Scan_table = ({ table_name, url }) => {
  ...
  const readOnClick = (name, key) => {
    Axios.put("http://34.68.101.191:8000/put/Scan_read", {
      business_name: name,
      key: key,
    });
  };
  ...
  const deleteOnClick = (key) => {
    Axios.delete("http://34.68.101.191:8000/delete/Scan_table", {
      data: {
        index: key,
      },
    });
  ...
  const [stateNames, setStateNames] = useState(
    table_name === "all_client"
      ? [
          { name: "기본스캔 작성중", isChecked: false },
          { name: "기본스캔 제출완료", isChecked: false },
          { name: "세부스캔 작성중", isChecked: false },
          { name: "세부스캔 II 작성중", isChecked: false },
          { name: "세부스캔 제출완료", isChecked: false },
          { name: "리포트", isChecked: false },
        ]
      : table_name === "defalt_client"
      ? [
          { name: "기본스캔 작성중", isChecked: false },
          { name: "기본스캔 제출완료", isChecked: false },
        ]
      ...
      ? [
          { name: "세부스캔 질문 작성중", isChecked: false },
          { name: "세부스캔 질문 제출완료", isChecked: false },
          { name: "세부스캔 II 질문 작성중", isChecked: false },
          { name: "세부스캔 II 질문 요청완료", isChecked: false },
          { name: "리포트 작성중", isChecked: false },
          { name: "리포트 제출완료", isChecked: false },
        ]
      : table_name === "detail_writer"
      ? [
          { name: "세부스캔 질문 작성중", isChecked: false },
          { name: "세부스캔 질문 제출완료", isChecked: false },
          { name: "세부스캔 II 질문 작성중", isChecked: false },
  ...
  useEffect(() => {
    ...
    if (table_name === "all_client") {
      key = "all_client";
    } else if (table_name === "defalt_client") {
      key = "defalt_client";
      ...
      key = "all_writer";
    } else if (table_name === "detail_writer") {
      key = "detail_writer";
    } else if (table_name === "report_writer") {
      key = "report_writer";
    }
    Axios.get("http://34.68.101.191:8000/get/Scan_table", {
      params: { key: key },
    }).then((response) => {
  ...
  return (
    <>
      <div className="table_top">
        ...
        {table_name === "all_client" || table_name === "defalt_client" ? (
          <NavLink to="/Scan_new_client">
              ...
              <TableCell align="center">
                ...
                <div className="table_filter">
                  ...
                  {showFilter && (
                    <div
                      className={
                        table_name.slice(-6) === "writer"
                          ? "table_filter_box focus_table_filter_box"
                          : "table_filter_box"
                      }
                    >
